...
class Validator
{
    const HTTP_HTTPS_PATTERN = "/^(http:\/\/www\.|https:\/\/www\.|http:\/\/|https:\/\/)?[a-z0-9]+([\-\.]{1}[a-z0-9]+)*\.[a-z]{2,5}(:[0-9]{1,5})?(\/.*)?$/";
     ...
     * @return bool
     */
    public static function validate(string $code = 'email', string $string = ''): bool
    {
        return preg_match(self::choosePattern($code), $string);
    }
     ...
     * @return string
     */
    private static function choosePattern(string $code = 'email'): string
    {
        $pattern = '';
        ...
        {
            case 'email':
                $pattern = self::HTTP_HTTPS_PATTERN;
                break;
            case 'internal_links':
